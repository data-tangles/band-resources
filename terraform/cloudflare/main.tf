...
resource "cloudflare_dns_record" "a_record_4" {
  zone_id = var.cloudflare_zone_id
...
resource "cloudflare_dns_record" "a_record_5" {
  zone_id = var.cloudflare_zone_id
  name    = var.a_record_5_name
  content = var.a_record_5_value
  type    = "A"
  ttl     = 1
  proxied = "true"
  comment = local.comment
...
resource "cloudflare_dns_record" "txt_record_3" {
  zone_id = var.cloudflare_zone_id
  name    = var.txt_record_3_name
  content = var.txt_record_3_value
  type    = "TXT"
  ttl     = 3600
  comment = local.comment
}
